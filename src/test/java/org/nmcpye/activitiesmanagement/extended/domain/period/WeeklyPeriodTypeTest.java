...
class WeeklyPeriodTypeTest {
    ...
    void testCreatePeriod() {
        ...
        assertFalse(startDate.isAfter(p.getStartDate().getTime())); // "start date after given date",
        assertFalse(endDate.isAfter(p.getEndDate().getTime())); // "end date before given date",
    ...
    @Test
    void testGeneratePeriodsWithCalendar() {
    ...
    void testGetRewindedDate() {
        assertEquals(new DateTime(2020, 1, 3, 0, 0).toDate(), periodType.getRewindedDate(new DateTime(2020, 1, 24, 0, 0).toDate(), 3));

        assertEquals(new DateTime(2020, 1, 15, 0, 0).toDate(), periodType.getRewindedDate(new DateTime(2020, 1, 1, 0, 0).toDate(), -2));
    }
}
