...
import org.nmcpye.activitiesmanagement.extended.common.hibernate.HibernateIdentifiableObjectStoreEM;
...
public class HibernatePeriodStoreEM extends HibernateIdentifiableObjectStoreEM<Period> implements PeriodStore {
    ...
    public HibernatePeriodStoreEM(JdbcTemplate jdbcTemplate, UserService userService) {
    ...
    public List<Period> getPeriodsBetweenDates(Date startDate, Date endDate) {
        ...
        TypedQuery<Period> typedQuery = getQuery(query).setParameter("startDate", startDate).setParameter("endDate", endDate);
    ...
    public List<Period> getPeriodsBetweenDates(PeriodType periodType, Date startDate, Date endDate) {
        ...
        TypedQuery<Period> typedQuery = getQuery(query)
            .setParameter("startDate", startDate)
            .setParameter("endDate", endDate)
    ...
    public List<Period> getPeriodsBetweenOrSpanningDates(Date startDate, Date endDate) {
        ...
        return getQuery(hql).setParameter("startDate", startDate).setParameter("endDate", endDate).getResultList();
    }
    ...
    public List<Period> getPeriodsByPeriodType(PeriodType periodType) {
        ...
        TypedQuery<Period> typedQuery = getQuery(query).setParameter("periodType", reloadPeriodType(periodType).getId());
        return getList(typedQuery);
    }
    ...
    public Period reloadPeriod(Period period) {
        ...
        if (entityManager.contains(period)) {
        ...
        Period storedPeriod = id != null ? entityManager.find(Period.class, id) : null;
    ...
    public PeriodType getPeriodType(Class<? extends PeriodType> periodType) {
        ...
        return entityManager.createQuery(query).setHint(QueryHints.CACHEABLE, cacheable).getSingleResult();
    }
    ...
    public List<PeriodType> getAllPeriodTypes() {
        ...
        return entityManager.createQuery(query).setHint(QueryHints.CACHEABLE, cacheable).getResultList();
    }

    @Override
    public PeriodType reloadPeriodType(PeriodType periodType) {
        if (periodType == null || entityManager.contains(periodType)) {
            return periodType;
        }
    ...
    public Period insertIsoPeriodInStatelessSession(Period period) {
        ...
        //        try {
        //            Serializable id = entityManager.persist(period);
    ...
    public void deleteRelativePeriods(RelativePeriods relativePeriods) {
        entityManager.remove(relativePeriods);
    }
}
