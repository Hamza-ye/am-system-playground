...
import org.nmcpye.activitiesmanagement.extended.common.adapter.JacksonRowDataSerializer;
    ...
    public Grid addColumn(List<Object> columnValues) {
        ...
        if (grid.size() != columnValues.size()) {
            throw new IllegalStateException(
                "Number of column values (" + columnValues.size() + ") is not equal to number of rows (" + grid.size() + ")"
            );
        }
