...
import org.nmcpye.activitiesmanagement.extended.common.calendar.exception.InvalidCalendarParametersException;
...
public class PersianCalendar extends AbstractCalendar {
    ...
    public DateTimeUnit toIso(DateTimeUnit dateTimeUnit) {
        if (dateTimeUnit.getYear() >= START_ISO.getYear() && dateTimeUnit.getYear() <= STOP_ISO.getYear()) {
            return new DateTimeUnit(
                dateTimeUnit.getYear(),
                dateTimeUnit.getMonth(),
                dateTimeUnit.getDay(),
                dateTimeUnit.getDayOfWeek(),
                true
            );
        }
        ...
        if (dateTimeUnit.getYear() > STOP_PERSIAN.getYear() || dateTimeUnit.getYear() < START_PERSIAN.getYear()) {
            throw new InvalidCalendarParametersException(
                "Illegal PERSIAN year, must be between " +
                START_PERSIAN.getYear() +
                " and " +
                STOP_PERSIAN.getYear() +
                ", was given " +
                dateTimeUnit.getYear()
            );
        }
    ...
    public DateTimeUnit fromIso(DateTimeUnit dateTimeUnit) {
        ...
        if (dateTimeUnit.getYear() < START_ISO.getYear() || dateTimeUnit.getYear() > STOP_ISO.getYear()) {
            throw new InvalidCalendarParametersException(
                "Illegal ISO year, must be between " +
                START_ISO.getYear() +
                " and " +
                STOP_ISO.getYear() +
                ", was given " +
    ...
    public DateTimeUnit minusYears(DateTimeUnit dateTimeUnit, int years) {
        DateTimeUnit result = new DateTimeUnit(
            dateTimeUnit.getYear() - years,
            dateTimeUnit.getMonth(),
            dateTimeUnit.getDay(),
            dateTimeUnit.getDayOfWeek()
        );
    ...
    private int getYearTotal(int year) {
        if (CONVERSION_MAP.get(year) == null) {
            throw new InvalidCalendarParametersException(
                "Illegal PERSIAN year, must be between " + START_PERSIAN.getYear() + " and " + STOP_PERSIAN.getYear() + " was given " + year
            );
        }
