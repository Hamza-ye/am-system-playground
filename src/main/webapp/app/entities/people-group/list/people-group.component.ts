...
export class PeopleGroupComponent implements OnInit {
  ...
  protected onSuccess(data: IPeopleGroup[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/people-group'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
