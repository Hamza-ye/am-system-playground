...
export class MalariaCasesReportComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.malariaCasesReportService
      .query({
        page: pageToLoad - 1,
  ...
  protected onSuccess(data: IMalariaCasesReport[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/malaria-cases-report'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
