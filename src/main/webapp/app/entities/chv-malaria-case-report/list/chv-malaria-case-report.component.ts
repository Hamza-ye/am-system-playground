...
export class ChvMalariaCaseReportComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.chvMalariaCaseReportService
      .query({
        page: pageToLoad - 1,
  ...
  protected onSuccess(data: IChvMalariaCaseReport[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/chv-malaria-case-report'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.chvMalariaCaseReports = data ?? [];
