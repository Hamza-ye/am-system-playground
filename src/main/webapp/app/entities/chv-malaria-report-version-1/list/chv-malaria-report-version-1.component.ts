...
export class ChvMalariaReportVersion1Component implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.chvMalariaReportVersion1Service
      .query({
        page: pageToLoad - 1,
  ...
  protected onSuccess(data: IChvMalariaReportVersion1[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/chv-malaria-report-version-1'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.chvMalariaReportVersion1s = data ?? [];
